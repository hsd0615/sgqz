...
function calcWeights(level, part) {
  const c=Math.min(8,Math.floor(level/25)+Math.floor(part/10)+1);
  const w=[];for(let q=1;q<=10;q++){const d=Math.abs(q-c);if(q>=9)w[q]=q===9?3:1.5;else w[q]=Math.max(1,25-d*8)}
  const t=w.slice(1).reduce((a,b)=>a+b,0);return{centerQ:c,weights:w.slice(1).map(x=>x/t*100)};
}
...
h+='<h2>品质加权</h2><p class=formula>centerQ=min(8,floor(level/25)+floor(part/10)+1)</p>';
h+='<p>rawW[q]=max(1,25-|q-centerQ|×8); Q9权重3; Q10权重1.5</p>';
