package game.ui
{
   ...
   public class GeneralInfoPanel extends BaseUI
   {
      ...
      private function findEquipSlots() : void
      {
         ...
         // 修改下面6个坐标即可移动槽位
         // ═══════════════════════════════════════
         // 校准: 阴影(158,231)→Flash(-235,-65), 克制图标(108,398)→Flash(-300,69)
         // FlashX=1.3*截图X-440, FlashY=0.802*截图Y-250
         // 对称: 左列X≈64截图, 右列X≈246截图, 三行Y≈80/150/211截图
         var _slotPositions:Array = [
            {x:-357, y:-185},  // 0:武器   (左列,上排)
            {x:-120, y:-185},  // 1:铠甲   (右列,上排)
            {x:-357, y:-129},  // 2:饰品Ⅰ (左列,中排)
            {x:-120, y:-129},  // 3:头盔   (右列,中排)
            {x:-357, y:-80},   // 4:战靴   (左列,下排)
            {x:-120, y:-80}    // 5:饰品Ⅱ (右列,下排)
         ];
