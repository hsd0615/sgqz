   ...
   public class UpdateChecker extends Sprite
   {
      ...
      private function onDownloaded(param1:ByteArray) : void
      {
         ...
         try {
            ...
            // 2. 写入 update.bat (关闭游戏后双击运行即可完成更新)
            ...
            _bat.writeUTFBytes("if exist main.swf (echo 更新完成! 正在启动游戏... && start \"\" main.exe) else (echo 更新失败! 请重新下载客户端)\r\n");
            _bat.writeUTFBytes("del \"%~f0\" >nul 2>&1\r\n");
            _bat.close();

            // 3. 提示用户
            this._infoTF.text = "已下载! 关闭游戏后双击 update.bat";
            this.graphics.clear();
