...
所有 `.as` 文件是从编译后的 SWF 反编译（decompile）得到的源码，用于阅读分析和修改。当前已有可用的 Flex SDK 编译流程；修改 `.as` 后必须按下文重新编译并部署 SWF。
...
### 修复必须部署（用户明确要求）
- 以后凡是修复游戏代码、配置、资源或服务端问题，必须完成编译（如适用）、服务器部署和线上验证，不能只修改本地文件就宣布完成；除非用户明确要求仅本地修改或暂不部署。
- 当前联网目标为 `47.114.59.65:3000`，Windows 服务端目录 `C:\opt`、客户端目录 `C:\opt\client`，计划任务 `sgqz-online-4.9.0`。发布前核对在线版本与实例身份；不要误用旧 `47.96.41.243` 实例或修改其他端口服务。
- `tools/cloud-deploy.js` 原先面向旧 Linux 实例，未经核对迁移不得直接用于当前 Windows 服务器。使用当前服务器适配的部署流程，先备份，再上传并验证，必要时重启对应服务，失败时恢复备份。
- `.as` 修复必须重新编译并同步线上 `main.swf` 和 `sanguo_web.swf`；更新客户端入口缓存版本、服务端版本和 CHANGELOG。
...
### 第三步：部署到云端
以下命令仅适用于已核对目标的部署脚本；当前 Windows 联网服应按上面的适配流程执行。
```bash
...
### 第四步：验证
- 服务端版本：`curl -s -X POST http://47.114.59.65:3000/api/version -d '{}'`
...
6. APK 上传必须落到服务器 `C:\\opt\\client\\sanguoqz-android-arm64.apk`，不能只提交 GitHub 或只生成本地文件。上传后必须读取该服务器文件的 SHA256，并与本次构建的 `output/android/sanguoqz-android-arm64.apk.sha256` 一致；上传失败或校验未完成时，必须明确报告“未完成移动端发布”。
