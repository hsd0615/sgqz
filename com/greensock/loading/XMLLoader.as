package com.greensock.loading
{
   ...
   public class XMLLoader extends DataLoader
   {
      ...
      protected static function _parseVars(param1:XML) : Object
      {
         ...
         var _loc6_:XML = null;
         var _loc7_:Object = {};
         var _loc8_:XMLList = param1.attributes();
         for each(_loc6_ in _loc8_)
         {
            _loc2_ = String(_loc6_.name());
            _loc4_ = _loc6_.toString();
            if(_loc2_ != "url")
            {
               if(_loc2_ == "context")
               {
                  _loc7_.context = new LoaderContext(true,_loc4_ == "own" ? ApplicationDomain.currentDomain : (_loc4_ == "separate" ? new ApplicationDomain() : new ApplicationDomain(ApplicationDomain.currentDomain)),null);
               }
               else
               {
                  _loc3_ = typeof _varTypes[_loc2_];
                  if(_loc3_ == "boolean")
                  {
                     _loc7_[_loc2_] = Boolean(_loc4_ == "true" || _loc4_ == "1");
                  }
                  else if(_loc3_ == "number")
                  {
                     _loc7_[_loc2_] = Number(_loc4_);
                  }
                  else
                  {
      ...
      public static function parseLoaders(param1:XML, param2:LoaderMax, param3:LoaderMax = null) : void
      {
         ...
         {
            if((_loc6_ = String(_loc10_.name()).toLowerCase()) == "loadermax")
            {
               _loc5_ = param2.append(new LoaderMax(_parseVars(_loc10_))) as LoaderMax;
               if(param3 != null && Boolean(_loc5_.vars.load))
               {
            ...
            else
            {
               if(_loc6_ in _types)
               {
                  ...
                  if(param3 != null && Boolean(_loc4_.vars.load))
                  {
