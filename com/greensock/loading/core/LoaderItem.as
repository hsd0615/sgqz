   ...
   public class LoaderItem extends LoaderCore
   {
      ...
      protected function _prepRequest() : void
      {
         ...
         if(Boolean(this.vars.noCache) && (!_isLocal || this._url.substr(0,4) == "http"))
         {
            ...
            {
               this._request.data = new URLVariables();
            }
            this._request.data.cacheBusterID = _cacheID++;
         }
      }
      ...
      override public function auditSize() : void
      {
         ...
         if(this._auditStream == null)
         {
            ...
            _loc1_ = new URLRequest(this._url);
            if(!_isLocal || this._url.substr(0,4) == "http")
            {
               _loc1_.data = new URLVariables("cacheBusterID=" + _cacheID++ + "&purpose=audit");
            }
      ...
      protected function _auditStreamHandler(param1:Event) : void
      {
         ...
         {
            if(this.vars.alternateURL != undefined && this.vars.alternateURL != "" && this._url != this.vars.alternateURL)
            {
