package com.greensock.loading
{
   ...
   public class LoaderMax extends LoaderCore
   {
      ...
      override protected function _calculateProgress() : void
      {
         ...
         while(--_loc3_ > -1)
         {
            _loc1_ = this._loaders[_loc3_];
            _loc2_ = _loc1_.status;
            if(_loc2_ <= LoaderStatus.COMPLETED || !this.skipPaused && _loc2_ == LoaderStatus.PAUSED || !this.skipFailed && _loc2_ == LoaderStatus.FAILED)
            {
               _cachedBytesLoaded += _loc1_.bytesLoaded;
               _cachedBytesTotal += _loc1_.bytesTotal;
            }
         }
         _cacheIsDirty = false;
      }
      ...
      protected function _cancelActiveLoaders() : void
      {
         ...
         while(--_loc2_ > -1)
         {
            _loc1_ = this._loaders[_loc2_];
            if(_loc1_.status == LoaderStatus.LOADING)
            {
               delete this._activeLoaders[_loc1_];
               this._removeLoaderListeners(_loc1_,false);
               _loc1_.cancel();
            }
         }
      }
      ...
      {
         var _loc4_:LoaderItem = null;
         var _loc5_:Array = this.getChildren(param3,true);
         var _loc6_:int = int(_loc5_.length);
         while(--_loc6_ > -1)
         {
            _loc4_ = _loc5_[_loc6_];
            _loc4_.url = _loc4_.url.split(param1).join(param2);
            if("alternateURL" in _loc4_.vars)
            {
               _loc4_.vars.alternateURL = _loc4_.vars.alternateURL.split(param1).join(param2);
            }
         }
      }
      ...
      protected function _auditSize(param1:Event = null) : void
      {
         ...
         var _loc4_:uint = this._loaders.length;
         var _loc5_:int = this.skipPaused ? LoaderStatus.COMPLETED : LoaderStatus.PAUSED;
         var _loc6_:int = 0;
         while(_loc6_ < _loc4_)
         {
            _loc2_ = this._loaders[_loc6_];
            if(!_loc2_.auditedSize && _loc2_.status <= _loc5_)
            {
               if(!_loc3_)
               {
                  _loc2_.addEventListener("auditedSize",this._auditSize,false,0,true);
               }
               _loc3_ = true;
               _loc2_.auditSize();
            }
      ...
      {
         var _loc2_:uint = 0;
         var _loc3_:LoaderCore = param1.target as LoaderCore;
         this._loaders.splice(this.getChildIndex(_loc3_),1);
         this._loaders.unshift(_loc3_);
         if(_status == LoaderStatus.LOADING && _loc3_.status <= LoaderStatus.LOADING && !(_loc3_ in this._activeLoaders))
         {
            this._cancelActiveLoaders();
            _loc2_ = this.maxConnections;
            this.maxConnections = 1;
            this._loadNext(null);
            this.maxConnections = _loc2_;
         }
      }
      ...
      public function get rawProgress() : Number
      {
         ...
         while(--_loc4_ > -1)
         {
            _loc1_ = LoaderCore(this._loaders[_loc4_]).status;
            if(_loc1_ != LoaderStatus.DISPOSED && !(_loc1_ == LoaderStatus.PAUSED && this.skipPaused) && !(_loc1_ == LoaderStatus.FAILED && this.skipFailed))
            {
               _loc3_++;
               _loc2_ += LoaderCore(this._loaders[_loc4_]).progress;
            }
         }
         return _loc3_ == 0 ? 0 : _loc2_ / _loc3_;
      }
   }
}
