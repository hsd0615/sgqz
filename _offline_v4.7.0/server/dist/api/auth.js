...
router.post('/login', (req, res) => {
    ...
    if (userID && password) {
        // 带密码：验证登录
        let player = repository_1.PlayerRepo.findByUserIdAndPassword(String(userID), String(password));
        if (player) {
            const token = repository_1.PlayerRepo.generateToken(player.id);
            // 加载全部武将数据（根因已修复：AES解密长字符串问题已解决）
            const allGenerals = repository_1.GeneralRepo.findByPlayerId(player.id);
            const armyModel = allGenerals.map((g) => ({
            ...
            const chooseCodes = allGenerals.filter((g) => g.is_deployed).map((g) => g.code);
            response.data = {
            ...
            // 密码错误或用户不存在
            const existingPlayer = repository_1.PlayerRepo.findByUserId(String(userID));
            if (existingPlayer) {
        ...
        // 无密码：新玩家
        const existing = repository_1.PlayerRepo.findByUserId(String(userID));
        if (existing) {
            response.success = false;
...
router.post('/register', (req, res) => {
    ...
    const player = repository_1.PlayerRepo.create(String(userID), roleName, imageID, agent || '4399', password || '');
    // 初始武将（三国Q战新春版）
...
router.get('/players', (req, res) => {
    const allPlayers = (0, database_1.getAll)('players');
...
router.post('/active', (req, res) => {
    ...
    const player = repository_1.PlayerRepo.findByUserId(String(userID));
    const response = {
        success: true,
        stamp: req.body.stamp,
