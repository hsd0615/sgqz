...
import { PlayerRepo, LeitaiRepo } from '../db/repository';
...
router.post('/list', (req: Request, res: Response) => {
  ...
  const rooms = LeitaiRepo.findAll();
...
router.post('/be-master', (req: Request, res: Response) => {
  ...
  const room = LeitaiRepo.findById(parseInt(rID));
  if (!room) { response.success = false; response.message = '房间号错误!'; res.json(response); return; }
  ...
  if (room.room_type === 1 && player.money < room.room_price) {
    response.success = false; response.message = '金币不足'; res.json(response); return;
  }
  if (room.room_type === 2 && player.exploit < room.room_price) {
    response.success = false; response.message = '功勋不足'; res.json(response); return;
  }
  if (room.room_type === 3 && player.dianka < room.room_price) {
    response.success = false; response.message = '点卡不足'; res.json(response); return;
  }

  // Deduct cost
  if (room.room_type === 1) PlayerRepo.update(player.id, { money: player.money - room.room_price } as any);
  else if (room.room_type === 2) PlayerRepo.update(player.id, { exploit: player.exploit - room.room_price } as any);
  else PlayerRepo.update(player.id, { dianka: player.dianka - room.room_price } as any);

  // Update room
  LeitaiRepo.update(room.r_id, {
    room_status: 1,
    master_id: player.id,
  ...
  const updatedRoom = LeitaiRepo.findById(room.r_id)!;
...
router.post('/be-slave', (req: Request, res: Response) => {
  const { roleID, rID } = req.body;
  ...
  let costPaid = false;
  if (room.room_type === 1 && player.money >= room.room_price) {
    PlayerRepo.update(player.id, { money: player.money - room.room_price } as any);
    costPaid = true;
  } else if (room.room_type === 2 && player.exploit >= room.room_price) {
    PlayerRepo.update(player.id, { exploit: player.exploit - room.room_price } as any);
    costPaid = true;
  } else if (room.room_type === 3 && player.dianka >= room.room_price) {
    PlayerRepo.update(player.id, { dianka: player.dianka - room.room_price } as any);
    costPaid = true;
  }
...
router.post('/fight-over', (req: Request, res: Response) => {
