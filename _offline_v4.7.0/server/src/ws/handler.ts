...
import { ClientSession, WSMessage } from '../models/types';
import { PlayerRepo } from '../db/repository';

// ============== Global State ==============
const sessions: Map<string, ClientSession> = new Map();     // peerID → ClientSession
const clientToWS: Map<WebSocket, ClientSession> = new Map(); // WS → ClientSession
const rooms: Map<string, Set<string>> = new Map();          // roomName → Set<peerID>

function getOrCreateRoom(name: string): Set<string> {
  if (!rooms.has(name)) {
...
// ============== Auth ==============
function handleAuth(ws: WebSocket, msg: WSMessage): void {
...
// ============== Room Management ==============
function handleJoinRoom(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) { sendTo(ws, { type: 'error', message: '请先认证' }); return; }
...
function handleLeaveRoom(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;
...
// ============== Chat ==============
function handleChat(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) { sendTo(ws, { type: 'error', message: '请先认证' }); return; }
...
// ============== Neighbor List ==============
function handleNeighborList(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;

  const roomName = msg.room;
  const room = rooms.get(roomName);
  if (!room) { sendTo(ws, { type: 'neighbor_list', neighbors: [] }); return; }
...
// ============== Battle (P2P Relay) ==============
function handleBattleRequest(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;
...
function handleBattleAccept(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;
...
function handleBattleDecline(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;
...
function handleBattleAction(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;
...
function handleBattleResult(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;
...
// ============== Server Info ==============
function handleServerInfo(ws: WebSocket, msg: WSMessage): void {
...
function handleP2PMessage(ws: WebSocket, msg: WSMessage): void {
  const session = clientToWS.get(ws);
  if (!session) return;
...
function makePeerInfo(session: ClientSession): any {
  return {
    pID: session.peerId,
