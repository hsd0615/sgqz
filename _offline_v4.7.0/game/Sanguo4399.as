   ...
   public class Sanguo4399 extends Sprite
   {
      ...
      private function loadPlayerList() : void
      {
         if(Config.OFFLINE_MODE)
         {
            ...
            try
            {
               var localFile:File = File.applicationStorageDirectory.resolvePath("data.json");
               var localStream:FileStream = new FileStream();
               localStream.open(localFile, FileMode.READ);
               var localData:Object = com.adobe.serialization.json.JSON.decode(localStream.readUTFBytes(localStream.bytesAvailable));
               localStream.close();
               var localRole:Object = localData.roleModel || {};
               _playerList.push({"userID":localRole.userID || "offline", "roleName":localRole.roleName || "单机玩家", "level":localRole.level || 1, "money":localRole.money || 0, "imageID":localRole.imageID || 1});
            }
            catch(localError:Error)
            {
               _playerList.push({"userID":"offline", "roleName":"单机玩家", "level":1, "money":0, "imageID":1});
            }
            renderPlayerList();
            return;
         }
      ...
      private function showRegisterPanel() : void
      {
         ...
         regBtn.addEventListener(MouseEvent.CLICK, function(e:MouseEvent):void {
            ...
            if(uid == "" || name == "") {
               showStatus("军号、将军名 缺一不可!", 0xFF5555); return;
            }
      ...
      private function doLogin() : void
      {
         ...
         // 单机版只用用户名识别本地存档，密码字段保留兼容旧界面但不参与校验。
         _loginPassword = "";
         saveConfig();

         if(Config.OFFLINE_MODE)
         {
            var offlineReq:Object = {};
            offlineReq.head = Head.HTTP_NEW_LOGIN;
            offlineReq.userID = _loginUserID;
            offlineReq.password = _loginPassword;
            AESController.getInstance().sendJSON(offlineReq, this.sendLoginResponse);
            return;
         }
      ...
      private function xmlChildCompleteHandler(param1:LoaderEvent) : *
      {
         ...
         else if(param1.target.name == "game01.data.shop")
         {
            ...
            if(!Config.IS_WEB && !Config.OFFLINE_MODE)
            {
         ...
         if(param1.target.name == "game01.data.tianfu" && !Config.OFFLINE_MODE)
         {
      ...
      private function createFightNow() : void
      {
         ...
         var _bgUrl:String = (int(_data.part) == 11 || int(_data.part) == 12) ? "battle_bg.png" : "bg/" + _pn + ".png";
