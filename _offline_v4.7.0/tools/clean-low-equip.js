...
let br=0,er=0;
db.bagItems=db.bagItems.filter(b=>{if(!b.code||!b.code.startsWith("proto_4_"))return true;if(lowQ.has(b.code)){br++;return false}return true});
db.generals.forEach(g=>{for(let s=1;s<=6;s++){const e=g["equip"+s];if(e&&e!=="0"&&lowQ.has(e)){g["equip"+s]="0";er++}}});
fs.writeFileSync("/opt/data/sanguo.json",JSON.stringify(db));
console.log("bag:"+br+" equip:"+er);
