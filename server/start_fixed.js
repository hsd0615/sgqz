...
function handleRequest(socket, req) {
  ...
  if (url === '/api/admin/upload' && method === 'POST') {
    if (!data || !data.key || data.key !== process.env.SGQZ_ADMIN_KEY) {
  ...
  if (url === '/api/version') {
    var _ts = Date.now();
    return jsonRawResponse(socket, { success: true, version: getClientVersion(), downloadUrl: 'http://47.114.59.65:3000/client/main.swf?v=' + _ts, mobileDownloadUrl: 'http://47.114.59.65:3000/client/sanguoqz-android-arm64.apk?v=' + _ts });
  }

  // 更新公告 - 返回最近版本更新内容（面向玩家）
  if (url === '/api/changelog') {
    return jsonRawResponse(socket, { success: true, entries: [
      { version: '4.0.10', title: '🔧 翻牌界面图标缩放修复+天赋点卡漏洞修复',
  ...
  if (url === '/api/fuben/flip') {
    ...
    if (fpResult[0] === '3') {
      ...
      resp.data.general = Object.assign({}, sg, {
        id: sg.general_id,
        title: 0,
        name: sg.name,
        kezhi: getKezhiStr(sg)
      });
      delete p._xiongnuSuperBoss;
  ...
  if (url.startsWith('/api/general/')) {
    ...
    if (headCode === 10025) {
      ...
      if (g) {
        ...
        if (isReroll && (p.dianka||0) < 100) return jsonRawResponse(socket, { success: false, message: '点卡不足，无法重洗天赋' });
        var tfPool = isReroll
  ...
  // ============ 远程管理 ============
  const ADMIN_KEY = process.env.SGQZ_ADMIN_KEY || require('crypto').randomBytes(32).toString('hex');
