...
function handleRequest(socket, req) {
  ...
  // 客户端版本号
  if (url === '/api/version') {
    return jsonRawResponse(socket, { success: true, version: '2.9.5', downloadUrl: 'http://47.96.41.243:3000/client/main.swf' });
  }
  ...
  if (url === '/api/admin/status') {
    ...
    return jsonRawResponse(socket, { success: true, uptime: Math.floor(uptime), memoryMB: Math.floor(mem.heapUsed/1024/1024), version: '2.9.5' });
  }
