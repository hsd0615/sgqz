...
const tcpToSession = new Map();
const rooms = new Map();
exports.rooms = rooms;
function setupTCPServer(port = 3001) {
    const server = net_1.default.createServer((socket) => {
        ...
        socket.on('data', (data) => {
            ...
            while (true) {
                ...
                if (session.buffer.length < session.expectedLength)
                    break;
...
function handleTCPMessage(session, msg) {
...
function handleAuth(session, msg) {
    const { roleID, token, roleName, level, imageId } = msg;
    const player = roleID ? repository_1.PlayerRepo.findByRoleId(String(roleID)) : undefined;
    if (!player && roleID) {
...
function handleJoinRoom(session, msg) {
    ...
    for (const pid of room) {
        ...
        if (other)
            sendTCP(other, { type: 'neighbor_join', peer: makePeerInfo(session) });
    }
...
function handleLeaveRoom(session, msg) {
    ...
    if (room) {
        ...
        if (room.size === 0)
            ...
            for (const pid of room) {
                const other = sessions.get(pid);
                if (other)
                    sendTCP(other, { type: 'neighbor_leave', peerId: session.peerId });
            }
        }
    }
...
function handleChat(session, msg) {
    ...
    for (const pid of room) {
        if (pid !== session.peerId) {
            const other = sessions.get(pid);
            if (other)
                sendTCP(other, chatMsg);
        }
    }
}
function handleNeighborList(session, msg) {
    const room = rooms.get(msg.room);
    if (!room) {
...
function handleBattleRequest(session, msg) {
...
function handleBattleAccept(session, msg) {
...
function handleBattleDecline(session, msg) {
    const opponent = sessions.get(msg.fromPeerId);
    if (opponent) {
...
function handleBattleAction(session, msg) {
...
function handleBattleEnd(session, msg) {
    const opponent = session.farPeerId ? sessions.get(session.farPeerId) : undefined;
    if (opponent) {
        opponent.farPeerId = null;
    }
...
function handleServerInfo(session) {
    sendTCP(session, {
