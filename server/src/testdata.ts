...
export function createTestAccounts() {
  // 检查是否已存在
  const existing = PlayerRepo.findByUserId('gm_admin');
  if (existing) {
  ...
  const p1 = PlayerRepo.create('gm_admin', 'GM管理员', 1, '4399', 'admin123');
  PlayerRepo.update(p1.id, {
    level: 220,
    money: 99999999,
    dianka: 999999,
    exploit: 99999999,
  ...
  for (const g of superGenerals) {
    ...
    const general = GeneralRepo.create(p1.id, {
    ...
    GeneralRepo.setDeployed(p1.id, [general.general_id]);
    chooseCodes.push(g.code);
  }

  // Add some bag items
  BagItemRepo.updateOrCreate(p1.id, 'proto_1_1', 999, 90001);
  BagItemRepo.updateOrCreate(p1.id, 'proto_1_2', 999, 90002);
  BagItemRepo.updateOrCreate(p1.id, 'proto_1_3', 999, 90003);
  BagItemRepo.updateOrCreate(p1.id, 'proto_1_4', 999, 90004);
  BagItemRepo.updateOrCreate(p1.id, 'proto_1_5', 999, 90005);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_1', 99, 90006);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_2', 99, 90007);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_3', 99, 90008);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_4', 99, 90009);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_5', 99, 90010);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_6', 99, 90011);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_7', 99, 90012);
  BagItemRepo.updateOrCreate(p1.id, 'proto_2_8', 99, 90013);
  BagItemRepo.updateOrCreate(p1.id, 'proto_3_1', 99, 90014);
  BagItemRepo.updateOrCreate(p1.id, 'proto_3_2', 99, 90015);
  BagItemRepo.updateOrCreate(p1.id, 'proto_3_3', 99, 90016);
  BagItemRepo.updateOrCreate(p1.id, 'proto_3_4', 99, 90017);

  console.log('[TestData] 测试账号1: gm_admin / admin123 (11个满级超级武将)');

  // ===== 测试账号2: test_pro (通关前3图) =====
  const p2 = PlayerRepo.create('test_pro', '测试高手', 1, '4399', 'pro123');
  PlayerRepo.update(p2.id, {
    level: 100,
    money: 500000,
    dianka: 50000,
    exploit: 500000,
  ...
  GeneralRepo.setDeployed(p2.id, proGenerals.map((g,i) => i+1));

  BagItemRepo.updateOrCreate(p2.id, 'proto_1_1', 50, 80001);
  BagItemRepo.updateOrCreate(p2.id, 'proto_2_1', 20, 80002);
  BagItemRepo.updateOrCreate(p2.id, 'proto_3_1', 20, 80003);
  BagItemRepo.updateOrCreate(p2.id, 'proto_3_4', 20, 80004);

  console.log('[TestData] 测试账号2: test_pro / pro123 (通关4图, 80级武将)');

  // ===== 测试账号3: new_player (新玩家) =====
  const p3 = PlayerRepo.create('new_player', '新兵报到', 1, '4399', 'new123');
